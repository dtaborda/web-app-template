...
# Fallback when script is executed through .agents symlink path
if [ ! -d "$SKILLS_DIR" ] || [ ! -d "$REPO_ROOT/packages" ]; then
    ALT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
    if [ -d "$ALT_ROOT/skills" ] && [ -d "$ALT_ROOT/packages" ]; then
        REPO_ROOT="$ALT_ROOT"
        SKILLS_DIR="$REPO_ROOT/skills"
    fi
fi
...
# NOTE: Customize this for your project structure
get_agents_path() {
    local scope="$1"
    case "$scope" in
        root|project)       echo "$REPO_ROOT/AGENTS.md" ;;
        ui)                 echo "$REPO_ROOT/ui/AGENTS.md" ;;
        packages)           echo "$REPO_ROOT/packages/AGENTS.md" ;;
        db)                 echo "$REPO_ROOT/packages/db/AGENTS.md" ;;
        supabase)           echo "$REPO_ROOT/supabase/AGENTS.md" ;;
        *)          echo "" ;;
    esac
}
